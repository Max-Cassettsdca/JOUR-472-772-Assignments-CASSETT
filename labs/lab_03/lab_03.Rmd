...
```{r}
# Load in the Maryland state agency payments data, ensuring all rows are used to set types
md_payments <- read_csv("data/payments.csv")

# Load in the Maryland grants and loan data
grants_loans <- read_csv("data/md_grants_loans.csv")

# Clean up the column names
md_payments <- clean_names(md_payments)
grants_loans <- clean_names(grants_loans)
```
...
school_name_finder <- md_payments |> filter(str_detect(vendor_name,"SCHOOL")) |> group_by(vendor_name) |> summarize(count=n())
...
```{r}
capital_projects_findr <-md_payments |> filter(str())
