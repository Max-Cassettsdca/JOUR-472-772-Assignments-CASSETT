...
**A1: It uses the name earthquakes which is pretty broad. It's a feed file which means that its getting updated all the time so the answers in our code will change depending on th time we initiate the code.**
...
#damn it this the first qu4stion?????
...
#I know what i need to do i just dont have a strong en ough hold on the concept. Like i understand that my first step is to tell R that I need to focus on TWO collumns, mag and depth. But I need to get them to do it separately.
```
...
```{r}
 ...
 earthquakes|> group_by(locationSource)
```
...
earthquakes|> summarize(average of depth)
                        
                        


#the reason that there is a negative depth is because the earthquakes occur above sea level, in mountians and such. 
```
